...
ll powerSer(ll x,ll len){
    ...
    watch(x);
    watch(len);
    ll g = power(x,len,MOD);
    watch(g);
    ll ans = 1 -  power(x,len,MOD);
    ans /= 1-x;
    //ans = ((modInverse(1-x,MOD)) * ( 1- power(x,len,MOD)) )%MOD;

    ans = modDivide(power(x,len,MOD)-1,x-1,MOD);
    return ans;
}
...
int main(){
    ...
    //cout<<powerSer(998244351,1)<<endl;
    //cout<<power(2,8,MOD);
    testCases = 1;
    cin>>testCases; 
    while(testCases--){
        ...
        for(int i = 0;i < n;i++){
            //watch(cpn); 
            ll m1 = (moduloMultiplication(ans,power(a,2*cpn,MOD),MOD));
            ll m2 = (moduloMultiplication(ans,power(powerSer(x,i),2*cpn,MOD),MOD));
            watch(m1);
            watch(m2);
            watch(x);
            watch(i);
            watch(powerSer(x,i));
            ans = moduloMultiplication(ans,power(a,2*cpn,MOD),MOD);
            ans = moduloMultiplication(ans,power(powerSer(x,i),2*cpn,MOD),MOD);
            cpn--;
            watch(ans);
            //watch(MOD);
        }
