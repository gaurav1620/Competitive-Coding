...
ll blloll(string s){
    ...
    ll i;
    fo(i,n){
        ans += (s[n-1-i]-'0') * (pow(2,i));
    }
    return ans;
}
ll best(ll x,ll y){
    string xx = decToBinary(x) + decToBinary(y);
    string yy = decToBinary(y) + decToBinary(x);
    return blloll(xx) - blloll(yy);
}
ll testCases;
int main(){
    Ninja
    ...
    while(testCases--){
        ...
        ll ans = best(arr[0],arr[0]);

        fo(i,n){
            fo(j,n){
                if(ans < best(arr[i],arr[j])){
                    ans = best(arr[i],arr[j]);
                }
                ans = ans < best(arr[i],arr[j]) ? best(arr[i],arr[j]):ans;
            }
        }
        cout<<ans<<endl;
    }
