...
int main(){
    ...
    ll sqrtnum = sqrt(1000000000);

    bool arr[sqrtnum+1];
    fo(i,sqrtnum+1)arr[i] = true;

    for(i = 2;i <= (sqrtnum+1);i++){
        if(arr[i] == true){
            for(j = i*i;j <= (sqrtnum+1);j += i){
                arr[j] = false;
            }
        }
    }
...
*/
    ...
    while(t--){
        ...
        bool mark[b-a+2];
        fo(i,b-a+2)mark[i] = true;
        for(i = 0;i < v.size() && v[i]*v[i] <= b;i++){
            //cout<<"IN for : "<<v[i]<<endl;
            
            ll lowerLim = (a/v[i])*v[i];
            if(lowerLim < a){
                lowerLim += v[i];
            }
            
            for(j = lowerLim;j <= b;j += v[i]){
                //cout<<"Marked : "<<j<<" as false"<<endl;
                if(j != v[i])mark[j-a]=false;
            }

        }      

        for(i = max(a,2);i <= b;i++){
            if(mark[i-a] == true)cout<<i<<endl;
        }
