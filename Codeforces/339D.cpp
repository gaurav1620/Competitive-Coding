...
node* createTree(ll lef,ll rig,node* parr,bool toOr){
    if(lef == rig){
        node* gg = new node;
        gg->val = arr[lef];
    ...
    gg->left = createTree(lef,mid,gg,!toOr);
    gg->right = createTree(mid+1,rig,gg,!toOr);
    gg->parent = parr;
    if(toOr) gg->val = gg->left->val | gg->right->val;
    else     gg->val = gg->left->val ^ gg->right->val;
    return gg;
}
...
int main(){
    ...
    ind = 0;
    node* root = createTree(0,n-1,NULL,toOr);
    ...
    while(q--){
        ...
        while(cur != NULL){
            if(toOr)cur->val = cur->left->val | cur->right->val;
            if(!toOr)cur->val = cur->left->val ^ cur->right->val;
            toOr = !toOr;
            cur = cur->parent;
        }
