...
bool vis2[100005];
ll pep[100005];
ll nextGenPop[100005];
ll calcHapp[100005];
ll n;
ll totPeeps;

ll makeNexGen(ll ind){
    ll popu = pep[ind];
    vis2[ind] = 1;
    for(ll op : adj[ind]){
        if(!vis2[op])popu += makeNexGen(op);
    }
    nextGenPop[ind] = popu;
    return popu;
}
...
ll dfs(ll ind){
    ...
    for(ll v : adj[ind]){
        if(!vis[v]){
            ...
            if(pp == -1)return -1;
            badPeepsBelow += pp;
            goodPeepsBelow += nextGenPop[v] - badPeepsBelow;
        }
    }
    if(isLeaf){
        ll twox = pep[ind] + calcHapp[ind];
        if(twox%2)return -1;
        return pep[ind] - twox/2;
    }else{
        ll score = goodPeepsBelow - badPeepsBelow;
        if(score - 2*pep[ind] <= calcHapp[ind] &&  calcHapp[ind] <= score){
            if((score - calcHapp[ind])%2)return -1;
            return (score-calcHapp[ind])/2;
        }else{
            return -1;
        }
    }
