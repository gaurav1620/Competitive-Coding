...
bool numOverlaps(vector<pair<ll,ll>> v,ll a,ll b){
    ll ret = 0;
    bool found= true;
    for(int i= 0; i < v.size();i++){
        if(b >= v[i].first && b <= v[i].second){
            found == true;
            break;
        }else if(a <= v[i].second && a >= v[i].first){
            found == true;
            break;
        }
    }
...
int main(){
    ...
    while(testCases--){
        ...
        while(m--){
            ...
            ll a,b;cin>>a>>b;
            if(numOverlaps(v,a,b)){
                v.push_back({a,b});
            }
        }

        ll ans = v[0].second - v[0].first +1;
        for(int i = 1 ; i < v.size();i++){
            ll tot = v[i].second-v[i].first +1;
            if(v[i].first <= v[i-1].second){
                tot -= v[i-1].second - v[i].first+1;
            }
            ans += tot;
        }
        cout<<ans<<endl;
