...
ll ans(ll rem_n1, ll rem_n2, ll rem_k1,ll rem_k2){
    if(rem_n1 == 0 && rem_n2 == 0)return 1;
    if(rem_n1 < 0 || rem_n2 < 0)return 0;
    ll &dd = dp[rem_n1][rem_n2][rem_k1][rem_k2];
    if(dd != -1)return dd;
    dd = 0;
    
    if(rem_k1 > 0 && rem_n1 > 0)dd += ans(rem_n1-1,rem_n2,rem_k1-1,k2)%MOD;
    dd %= MOD;
    if(rem_k2 > 0 && rem_n2 > 0)dd += ans(rem_n1,rem_n2-1,k1,rem_k2-1)%MOD;
    dd %= MOD;
