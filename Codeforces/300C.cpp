...
long long powMod(int a,int b)
{
	...
	long long ret=powMod(a,b/2)%MOD;
	if(b%2)return (((ret*ret)%MOD)*a)%MOD;
	return (ret*ret) % MOD;
}

long long multInv(int a,int b)
{
	return (powMod(a,b-2)+MOD)%MOD;
}
...
int main(){
    ...
    ll ans = 0;
    for(i = 0;i <= n;i++){
        ll sumOfDigits = i*a + (n-i)*b;
        ll sn = to_string(sumOfDigits).length();
        map<ll,ll> mp;
        while(sumOfDigits){
            mp[sumOfDigits%10]++;
            sumOfDigits/=10;
        ...
        if(mp[a] + mp[b] == sn){
            ll N = n;
            ans +=fact[N]*multInv((fact[i]*fact[N-i])%MOD,MOD); 
            ans %= MOD;
        }
    }
    cout<<ans<<endl;
